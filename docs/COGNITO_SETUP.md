...
### 1. The Cognito User Pool lives in go-template, not here

The Cognito User Pool and Client are created and managed by the paired
[go-template](https://github.com/qsweber/go-template) backend's CDK stack (`cdk/cognito.go`), not
...
go-template's CDK stack outputs. If go-template's Cognito setup ever changes (e.g. the pool is
